...
public class HW3Servlet extends HttpServlet {
	...
	void case_2(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			resp.getWriter().println("Поля m и n длжны быть заполнены числами!");
		}
	}
	...
	void case_5(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (d > 0) {
				...
				x1 = -(int_b) / (2 * int_a);
				resp.getWriter().println(
						"<p>Уравнение имеет только один корень, поскольку дискриминант рабен 0!</p>"
								+ "<p>X=" + x1 + "</p>");
			} else if (d < 0) {
				resp.getWriter()
						.println(
								"<p>Уравнение не имеет корней, так как дискриминант меньше 0</p>");
			}
		} catch (NumberFormatException n) {
			resp.getWriter().println(
					"<p>В полях a, b и c должны быть числа</p>");
		}
	}
